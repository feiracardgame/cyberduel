...
(async () => {
  ...
  const fallback = rows[0].children[1].children[0];
  assert.equal(fallback.children[0].textContent, 'BE');
  assert.equal(fallback.children[1].textContent, 'Beto');
  const player = rows[1].children[1].children[0];
  assert.equal(player.children[0].tag, 'img');
  assert.equal(player.children[0].src, 'assets/fotosdeperfil/juggernaut_icon.png');
  assert.equal(player.children[0].alt, '');
  assert.equal(player.children[1].textContent, '<b>Ana</b>', 'Apelido permanece texto.');
